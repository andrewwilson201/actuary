...
#' losses |> mutate(l1 = layer_loss(year, amount, 10e6, 10e6, 1))
#' 
#' # calculate 10m xs 10m agg layer with 0 reinstatements
#' losses |> mutate(l1 = layer_loss(year, amount, 10e6, 10e6, 0, type = "aggregate"))
#' 
#' }
#' 
#' @export
  ...
  if (type == "occurrence") {
    ...
    dplyr::tibble(year = {{ year }}, loss = {{ loss }}) |>
      ...
      dplyr::mutate(ul_layer_loss = pmin(pmax(loss - retention, 0), limit),
                    ...
                    cumulative_loss_lag = lag(cumsum(ul_layer_loss), default = 0)) |> 
      dplyr::ungroup() |> 
      # calculate capped layer loss allowing for reinstatements
      dplyr::mutate(capped_layer_loss = if_else(cumulative_loss > limit * (reinstatements + 1),
