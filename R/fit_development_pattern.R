...
#' @param calculate_mack_se if set to TRUE then return the Mack estimates of the standard error of the reserves in the ultimates dataframe
#'
...
fit_development_pattern <- function(cohort_var, dev_var, weighting_var, data,
                                     dev_period_length = 1, dev_period_units = 12, exclude_last_diag = FALSE,
                                     num_periods = NULL, selected_curve = "weibull", smooth_from = NULL, future_dev_periods = 0,
  ...
  # if mack standard errors are requested but a BF or CC approach is being used or a tail factor has been provided then warn

  if(calculate_mack_se == TRUE & (!is.null(bf_prior) | !is.null(cc_decay_factor))) warning("Mack estimates of the standard error of the reserves have been requested but a BF or Cape Cod approach is being used. Mack estimates will not be appropriate.")
  if(calculate_mack_se == TRUE & tail_factor != 1) warning("Mack estimates of the standard error of the reserves have been requested but a tail factor is being used. Mack estimates will not be appropriate.")
  ...
  development_pattern <- data |>
    ...
    dplyr::summarise(denominator = sum({{ weighting_var }} * denominator_include, na.rm = TRUE),
                     ...
                     ata_ratio = list(dplyr::pick(ata_ratio, {{ weighting_var }}) |>
                                        tidyr::drop_na(ata_ratio) |>
                                        dplyr::rename(claim_value = 2))) |>
    dplyr::ungroup() |>
    ...
    dplyr::mutate(atu = rev(cumprod(rev(ata))),
                  pct_dev = 1 / atu) |>
    dplyr::select({{ dev_var }}, ata, atu, pct_dev, original_data_flag, ata_ratio) |>
    dplyr::rename(dev_period = {{ dev_var }})

  # fit weibull curve

  weibull_fit <- development_pattern |>
  ...
  exponential_fit <- development_pattern |>
    dplyr::filter(original_data_flag == 1) |>
    dplyr::mutate(dev_period = dplyr::row_number()) |>
    dplyr::mutate(ata_fit = dplyr::if_else(ata <= 1, NA_real_, ata),
                  y_t = log(ata_fit - 1),
  ...
  inv_power_fit <- development_pattern |>
    dplyr::filter(original_data_flag == 1) |>
    dplyr::mutate(dev_period = dplyr::row_number()) |>
    dplyr::mutate(ata_fit = dplyr::if_else(ata <= 1, NA_real_, ata),
                  y_t = log(ata_fit - 1),
  ...
  ultimates <- data |>
    ...
    dplyr::left_join(development_pattern |>
  ...
  monthly_data <- ultimates |>
    dplyr::mutate(dev_month = dev_period * {{ dev_period_units }},
                  pct_dev = {{ weighting_var }} / ultimate) |>
  ...
  if (calculate_mack_se == TRUE) {

    ## calculate Mack SE errors of the reserve if required

    # calculate variance parameter for Mack SE estimates

    development_pattern <- development_pattern |>
      dplyr::mutate(dof = max(dev_period * original_data_flag) - dev_period - 1) |>
      dplyr::rowwise()  |>
      dplyr::mutate(sigma = sum((ata_ratio$ata_ratio - ata) ^ 2 * ata_ratio$claim_value) / dof)  |>
      dplyr::ungroup() |>
      dplyr::mutate(extrapolated_sigma1 = dplyr::lag(sigma ^ 2 / dplyr::lag(sigma)),
                    extrapolated_sigma2 = dplyr::lag(sigma),
                    extrapolated_sigma3 = dplyr::lag(dplyr::lag(sigma)),
                    dplyr::across(extrapolated_sigma1:extrapolated_sigma3, \(x) tidyr::replace_na(x, 0)),
                    extrapolated_sigma = pmin(extrapolated_sigma1, pmin(extrapolated_sigma2, extrapolated_sigma3)),
                    sigma = dplyr::if_else(dof == 0, extrapolated_sigma, sigma)) |>
      dplyr::select(-extrapolated_sigma, -extrapolated_sigma1, -extrapolated_sigma2, -extrapolated_sigma3)

    # add Mack SE to ultimates

    ultimates <- ultimates |>
      dplyr::left_join(development_pattern |>
                         dplyr::select(dev_period, sigma, ata_ratio, selected_ata),
                       dplyr::join_by(dev_period)) |>
      dplyr::mutate(reserve = ultimate - {{ weighting_var }}) |>
      dplyr::arrange(dev_period) |>
      # calculate the multiplier at the beginning of the Mack formula for mse(R)
      dplyr::mutate(mult1 = sigma / selected_ata ^ 2) |>
      dplyr::ungroup() |>
      # calculate the prior incurred on each row
      dplyr::rowwise() |>
      dplyr::mutate(prior_claim_values = sum(ata_ratio$claim_value)) |>
      dplyr::ungroup() |>
      # calculate the future triangle values as a list column
      dplyr::mutate(
        wvar_col = {{ weighting_var }},
        projected_values = purrr::map(
          dplyr::row_number(),
          ~ {
            i <- .
            ...
            # multiply each partial product by the incurred to get all future values
            future_values <- wvar_col[i] * partial_products
            # get all future values of the Mack multiplier
            mack_multiplier <- mult1[i:dplyr::n()]
            # get future values of prior incurred
            prior_cy_values <- prior_claim_values[i:dplyr::n()]
            # return the result in a tibble
            dplyr::tibble(
              future_values = future_values,
              mack_multiplier = mack_multiplier,
              prior_cy_values = prior_cy_values
            ) |>
              dplyr::filter(mack_multiplier != 0)
          }
        )) |>
      # calculate the standard error
      dplyr::rowwise() |>
      dplyr::mutate(mack_se = sqrt(ultimate ^ 2 * sum(projected_values$mack_multiplier * (1 / projected_values$future_values + 1 / projected_values$prior_cy_values)))) |>
      # retain required values
      dplyr::select(cohort, dev_period, {{ weighting_var }}, pct_dev, ultimate, reserve, mack_se) |>
      dplyr::arrange(cohort)

  }

  # remove columns not needed in development_pattern

  development_pattern <- development_pattern |>
    dplyr::select(-original_data_flag, -ata_ratio)

  return(list(development_pattern = development_pattern, data = data |>
